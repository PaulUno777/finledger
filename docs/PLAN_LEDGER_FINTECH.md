...
| `ledger:admin` parent AGGREGATOR → comptes d'un `SUB_MERCHANT` direct | `platform:admin` superuser data-plane ; 3e token IdP obligatoire | Hiérarchie déjà serveur ; money paths restent `tenant_id` = path ([ADR-018](adr/ADR-018-parent-admin-child-accounts.md)) |
