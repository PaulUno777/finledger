...
if not defined JAR if exist "%REPO_ROOT%\finledger-cli\target\finledger-cli-0.2.0.jar" (
  set "JAR=%REPO_ROOT%\finledger-cli\target\finledger-cli-0.2.0.jar"
)
...
if not defined JAR (
  if exist "%REPO_ROOT%\mvnw.cmd" (
    ...
    popd
    if exist "%REPO_ROOT%\finledger-cli\target\finledger-cli-0.2.0.jar" (
      set "JAR=%REPO_ROOT%\finledger-cli\target\finledger-cli-0.2.0.jar"
    )
  )
)
